...
let rec check_cmd vs c =
  ...
  | Malloc v ->
      if List.mem v vs then true
      else (
        Printf.printf "`%s` not declared in scope!\n" v;
        false)
  | ProcedureCall (e1, e2) -> check_expr vs e1 && check_expr vs e2
  | Declare (s, c1) -> check_cmd (s :: vs) c1
  | Assign (v, e) ->
      if List.mem v vs then check_expr vs e
      else (
        Printf.printf "`%s` not declared in scope!\n" v;
        false)
  (* TODO: maybe create visible fields *)
  | FieldAssign (e1, e2, e3) ->
      check_expr vs e1 && check_expr vs e2 && check_expr vs e3
